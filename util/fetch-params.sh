...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/INFINCOINParams"
    else
        PARAMS_DIR="$HOME/.infincoin-params"
    fi
fi
...
Failed to fetch the INFINCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
INFINCOIN - fetch-params.sh

This script will fetch the INFINCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common INFINCOIN zkSNARK parameters. Note that it is
