...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("INFINCOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("INFINCOINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("INFINCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("infincoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running INFINCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("INFINCOINMiner exception");
    } catch (...) {
        LogPrintf("INFINCOINMiner exception");
    }

    LogPrintf("INFINCOINMiner exiting\n");
}
