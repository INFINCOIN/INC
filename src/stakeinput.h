...
#ifndef INFINCOIN_STAKEINPUT_H
#define INFINCOIN_STAKEINPUT_H
...
class CIncStake : public CStakeInput
{
private:
...
public:
    CIncStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CIncStake* NewIncStake(const CTxIn& txin);
