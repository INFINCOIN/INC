...
const char * const INFINCOIN_CONF_FILENAME = "infincoin.conf";
const char * const INFINCOIN_PID_FILENAME = "infincoin.pid";
const char * const INFINCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// INFINCOIN only features
// Masternode
...
#else
    const char* pszModule = "infincoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\INFINCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\INFINCOIN
// Mac: ~/Library/Application Support/INFINCOIN
// Unix: ~/.infincoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFINCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\INFINCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\INFINCOINParams
    // Mac: ~/Library/Application Support/INFINCOINParams
    // Unix: ~/.infincoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFINCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "INFINCOINParams";
#else
    // Unix
    return pathRet / ".infincoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/infincoin/sapling-spend.params";
        sapling_output = "/usr/share/infincoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/infincoin/sapling-spend.params";
            sapling_output = "/usr/local/share/infincoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", INFINCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", INFINCOIN_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty infincoin.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override infincoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", INFINCOIN_PID_FILENAME));
