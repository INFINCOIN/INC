...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select INC Outputs to Spend" :
            "Select Shielded INC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::INC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show INFINCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
