...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::INC);
    ...
    if (settings.contains("nAnonymizeinfincoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeinfincoinamount", settings.value("nAnonymizeinfincoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in infincoin.cpp during shotdown to also remove the window positions
