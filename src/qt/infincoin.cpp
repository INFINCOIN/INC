...
#include "config/infincoin-config.h"
#endif
...
#include "qt/infincoin/splash.h"
#include "qt/infincoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("infincoin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in infincoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in infincoin.qrc)
...
/** Class encapsulating INFINCOIN startup and shutdown.
...
/** Main INFINCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (INFINCOINGUI)
...
{
    window = new INFINCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &INFINCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &INFINCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(INFINCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(INFINCOINGUI::DEFAULT_WALLET);
        ...
        // INFINCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &INFINCOINGUI::handlePaymentRequest);
        connect(window, &INFINCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. INFINCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(infincoin_locale);
    Q_INIT_RESOURCE(infincoin);
    ...
    /// 6. Determine availability of data directory and parse infincoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("INFINCOIN"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("INFINCOIN"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // infincoin: links repeatedly have their payment requests routed to this process:
