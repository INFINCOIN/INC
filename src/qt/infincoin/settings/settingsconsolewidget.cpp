...
#include "qt/infincoin/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/infincoin/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(INFINCOINGUI* _window, QWidget* parent) : PWidget(_window, parent),
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid INFINCOIN Transparent Address
        std::vector<std::string> args;
    ...
    if (cmd.size() >= 18 && cmd.leftRef(16) == "exportsaplingkey") {
        // valid INFINCOIN Shield Address
        std::vector<std::string> args;
        parseCommandLineSettings(args, cmd.toStdString());
