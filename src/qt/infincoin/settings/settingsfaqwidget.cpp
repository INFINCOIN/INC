...
#include "qt/infincoin/settings/settingsfaqwidget.h"
#include "qt/infincoin/qtutils.h"
#include "qt/infincoin/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(INFINCOINGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableINC,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableINC,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("INFINCOIN is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. INFINCOIN incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("INFINCOIN utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of INFINCOIN to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableINCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received INFINCOIN requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12-15 minutes.")) +
        formatFAQParagraph(
            tr("Your INFINCOIN wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableINC->setText(unspendableINCContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of INFINCOIN with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("infincoin", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node INFINCOIN wallet with a "
               "requirement of 10,000 INC secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in INFINCOIN Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the INFINCOIN network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 INC per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 INC collateral "
    ...
    connect(ui->pushButton_UnspendableINC, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableINC); });
    ...
    if (parent)
        connect(parent, &INFINCOINGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
