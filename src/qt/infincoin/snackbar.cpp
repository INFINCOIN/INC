...
#include "qt/infincoin/forms/ui_snackbar.h"
#include "qt/infincoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(INFINCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &INFINCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
