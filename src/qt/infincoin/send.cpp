...
#include "qt/infincoin/send.h"
#include "qt/infincoin/forms/ui_send.h"
#include "qt/infincoin/addnewcontactdialog.h"
#include "qt/infincoin/qtutils.h"
#include "qt/infincoin/sendchangeaddressdialog.h"
#include "qt/infincoin/optionbutton.h"
#include "qt/infincoin/sendconfirmdialog.h"
#include "qt/infincoin/guitransactionsutils.h"
#include "qt/infincoin/loadingdialog.h"
...
SendWidget::SendWidget(INFINCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-inc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onINCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onINCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent INCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield INCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent INCs to shield."));
    }
}
...
void SendWidget::onINCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto incAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(incAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(incAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(incAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
