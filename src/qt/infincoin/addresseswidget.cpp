...
#include "qt/infincoin/forms/ui_addresseswidget.h"
#include "qt/infincoin/addresslabelrow.h"
#include "qt/infincoin/addnewaddressdialog.h"
#include "qt/infincoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(INFINCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto incAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(incAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(incAdd);
        if (walletModel->updateAddressBookLabels(incAdd, label.toUtf8().constData(),
