...
#include "qt/infincoin/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/infincoin/forms/ui_dashboardwidget.h"
#include "qt/infincoin/qtutils.h"
#include "qt/infincoin/sendconfirmdialog.h"
#include "qt/infincoin/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(INFINCOINGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareInc, "square-chart-inc");
    setCssProperty(ui->labelInc, "text-chart-inc");
    ...
    setCssProperty(ui->labelAmountInc, "text-stake-inc-disable");
    ...
    if (window)
        connect(window, &INFINCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("INC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair INC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            inc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalInc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesInc.append(inc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(103, 102, 102));
    set1->setColor(QColor(103, 102, 102));
    ...
    set0->append(chartData->valuesInc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalInc > 0) {
        setCssProperty(ui->labelAmountInc, "text-stake-inc");
    } else {
        setCssProperty(ui->labelAmountInc, "text-stake-inc-disable");
    }
    forceUpdateStyle({ui->labelAmountInc});
    ui->labelAmountInc->setText(GUIUtil::formatBalance(chartData->totalInc, nDisplayUnit));
