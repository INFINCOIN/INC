...
CIncStake* CIncStake::NewIncStake(const CTxIn& txin)
{
    ...
    return new CIncStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CIncStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CIncStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CIncStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CIncStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CIncStake::GetUniqueness() const
{
    //The unique identifier for a INC stake is the outpoint
...
const CBlockIndex* CIncStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CIncStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CIncStake::ContextCheck(int nHeight, uint32_t nTime)
{
