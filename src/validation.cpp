...
#if defined(NDEBUG)
#error "INFINCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uinc) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("infincoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> incInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                incInputs.push_back(stakeIn);
        }
        const bool hasINCInputs = !incInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasINCInputs) {
                    ...
                    for (const CTxIn& incIn : incInputs)
                        if(incIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasINCInputs) {
                            for (const CTxIn& stakeIn : incInputs)
