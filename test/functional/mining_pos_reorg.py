...
class ReorgStakeTest(infincoinTestFramework):
    ...
    def check_money_supply(self, expected_inc):
        # verify that nodes have the expected INC supply
        inc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(inc_supply, [DecimalAmt(expected_inc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that INC supply was properly updated after the reorgs
        self.log.info("Check INC supply...")
