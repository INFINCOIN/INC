...
*Setup instructions for a gitian build of INFINCOIN using a VM or physical system.*

Gitian is the deterministic build process that is used to build the INFINCOIN
...
to the INFINCOIN GitHub release page.
...
- [Building INFINCOIN](#building-infincoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/infincoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/infincoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/infincoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/infincoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/infincoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/infincoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/infincoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@infincoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/infincoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/infincoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/infincoin-osx-*-build.assert
```
