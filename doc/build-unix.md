UNIX BUILD NOTES
====================
Some notes on how to build INFINCOIN in Unix.

Note
---------------------
Always use absolute paths to configure and compile INFINCOIN and the dependencies,
...
This will build infincoin-qt as well, if the dependencies are met.
...
memory available when compiling INFINCOIN. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~infincoin/+archive/infincoin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:infincoin/infincoin
    sudo apt-get update
...
To build INFINCOIN without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build infincoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a infincoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip infincoind" to strip the debug
...
Security
--------
To help make your INFINCOIN installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, INFINCOIN should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, INFINCOIN may be compiled in
